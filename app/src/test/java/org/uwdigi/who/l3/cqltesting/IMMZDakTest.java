...
public class IMMZDakTest {
    ...
    @Test
    public void testIMMZIND12() {
        //measure, libraries, plandefinitions, and activitydefinitions
        L3InMemoryFhirRepository contentRepository;
        {
            FhirResourceLoader resourceLoader = new FhirResourceLoader(FhirContext.forR4Cached(), getClass(), List.of(Paths.get("immz", "content").toString()), true);
        ...
        {
            FhirResourceLoader resourceLoader = new FhirResourceLoader(FhirContext.forR4Cached(), getClass(), List.of(Paths.get("immz", "terminology").toString()), true);
            BundleBuilder builder = new BundleBuilder(FhirContext.forR4Cached());
            processResourcesToBundle(builder, resourceLoader.getResources());
        ...
        {
            FhirResourceLoader resourceLoader = new FhirResourceLoader(FhirContext.forR4Cached(), getClass(), List.of(Paths.get("immz", "IMMZIND12").toString()), true);
            BundleBuilder builder = new BundleBuilder(FhirContext.forR4Cached());
            processResourcesToBundle(builder, resourceLoader.getResources());
        ...
        MeasureReport report = measureProcessor.evaluateMeasure(
                Eithers.forLeft3(new CanonicalType("http://smart.who.int/immunizations-measles/Measure/IMMZIND12")),
                "2020-01-01", "2020-12-31", "population", null, null, null);
